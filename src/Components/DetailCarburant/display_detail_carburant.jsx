...
const DisplayDetCar = () => {
  ...
  const getDetails = useCallback(async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "carburant"));
      const detailData = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      // Filtrer les détails par UID si selectedUID n'est pas vide
  ...
  useEffect(() => {
    getDetails();
    document.title = "Details du carburant";
  ...
  return (
    <Box m="20px">
      <Header title="DETAILS DES CARBURANTS" />
      <Box
        m="40px 0 0 0"
        height="75vh"
        sx={{
